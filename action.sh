...
git clone git@${INPUT_GHES_HOST}:${INPUT_MANIFEST_ORG}/${INPUT_MANIFEST_REPO}.git ${INPUT_MANIFEST_NAME}

if [ "${INPUT_IMAGE_NAME}" == "" ]; then
    INPUT_IMAGE_NAME=${INPUT_MANIFEST_NAME}
fi

cd ${INPUT_MANIFEST_NAME}/${INPUT_MANIFEST_PATH}/${INPUT_MANIFEST_NAME}
OLD_IMAGE=$(cat kustomization.yaml | grep -A3 ${INPUT_IMAGE_NAME} | grep newTag | awk '{print $2}')
